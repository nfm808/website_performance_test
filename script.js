...
function createResultsTemplate() {
	let mobileTime = Math.round(DATA.mobile.timing.total) * 0.001;
	let desktopTime = Math.round(DATA.desktop.timing.total) * 0.001;
...
function displayMobileRender() {
	...
	$(".render_timeline").append(
		`<h4 class="close_div close_mobile"><a href="#">X</a></h4><div class="render_thumbnails"></div>`
	);
	closeTimelineDiv();
	for (let i = 0; i < info.length; i++) {
		src = DATA.mobile.audits["screenshot-thumbnails"].details.items[i].data;
		time =
			DATA.mobile.audits["screenshot-thumbnails"].details.items[i].timing *
			0.001;
		$(".render_thumbnails").append(
			`<div class="thumb_box"><img class="thumbnail"src="${src}" alt="website render by load time image"/><p>${time}s</p></div>`
		);
	}
}
...
function displayDesktopRender() {
	...
	let time = [];
	$(".render_timeline").append(
		`<h4 class="close_div"><a href="#">X</a></h4><div class="render_thumbnails"></div>`
	);
	closeTimelineDiv();
	for (let i = 0; i < info.length; i++) {
		src = DATA.desktop.audits["screenshot-thumbnails"].details.items[i].data;
		time =
			DATA.desktop.audits["screenshot-thumbnails"].details.items[i].timing *
			0.001;
		$(".render_thumbnails").append(
			`<div class="thumb_box"><img class="thumbnail" src="${src}" alt="website render by load time image" /><p>${time}s</p></div>`
		);
...
function handleSubmitForm() {
	$("#submit").click(function (e) {
		...
		if (validateForm() === true) {
			$("#js_contact").append(
				`<h3 class="error_response">Thank you and we will be in contact soon.</h3>`
			);
		} else {
			$("#js_contact").append(
				`<h3 class="error_response">Please check form and try again.</h3>`
			);
		}
	});
}
...
function drawChart() {
	...
	var chart = new google.visualization.Gauge(
		document.getElementById("chart_div_mobile")
	);
	var chartD = new google.visualization.Gauge(
		document.getElementById("chart_div_desktop")
	);
